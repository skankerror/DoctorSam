...
int Rs232::OpenSerial(string port)
{
  int res;
...
#ifdef DEBUG_RS232
  std::cerr << " Rs232::OpenSerial() " << port << " -> " << this->fd << " (" << res << ")" << std::endl;
#endif
  return res;
}

int Rs232::OpenSerial(string port, int flags)
{
  int res;
...
int Rs232::WaitingData(Pending pending/*=pendingInput*/, timeout_t timeout/*= TIMEOUT_INF*/)
{
  int status;
  struct timeval tv;
  fd_set grp;
...
int Rs232::SetSerialParams(long speed, int bits, int parity, int stops, int flow_control)
{
...
#ifdef DEBUG_RS232
  std::cerr << " Rs232::SetSerialParams() : " << result << std::endl;
  if(result < 0)
    perror(" Rs232::SetSerialParams() ");
#endif

  return result;
}

int Rs232::SetCustomBaudRate(int baud_rate/*=250000*/)
{
...
#ifdef DEBUG_RS232
  std::cerr << " Rs232::SetCustomBaudRate() : " << baud_rate << std::endl;
...
long Rs232::RecvBufferEX(BYTE *buffer, DWORD len, DWORD read_total_timeout)
{
  int retval = 0;
...
BYTE Rs232::RecvByte()
{
  ...
  if ( this->fd == INVALID_HANDLE_VALUE ) return ret;
  ret = read(this->fd,&carlu,1);

#ifdef DEBUG_RS232
  printf(" Rs232::RecvByte() : 0x%02X\n", (BYTE)carlu);
#endif
  return carlu;
}

long Rs232::SendBuffer(BYTE *buffer, DWORD len)
{
  DWORD i;
#ifdef DEBUG_RS232
  std::cerr << " Rs232::SendBuffer() : " ;
  for(i=0;i<len;i++)
    fprintf(stderr, "0x%02x ", *(buffer+i));
  fprintf(stderr, "\n");
#endif
...
long Rs232::SendByte(BYTE buffer)
{
  DWORD len = 1;

#ifdef DEBUG_RS232
  std::cerr << " Rs232::SendByte() : " ;
...
void Rs232::CloseSerial()
{
#ifdef DEBUG_RS232
  std::cerr << " Rs232::CloseSerial() " << std::endl;
#endif
  close(this->fd);
}

int Rs232::flushInput(void)
{
  if ( this->fd == INVALID_HANDLE_VALUE ) return 0;
...
void Rs232::Purge(void)
{
  flushInput();
  flushOutput();
}

int Rs232::SendBreak(void)
{
  if ( this->fd == INVALID_HANDLE_VALUE ) return 0;
...
void Rs232::WaitForTxEmpty()
{
  if ( fd != INVALID_HANDLE_VALUE )
    tcdrain(fd);
}

int Rs232::SetSerialBreak(int state)
{
  int result = E2ERR_OPENFAILED;
...
#ifdef DEBUG_RS232
  std::cerr << " Rs232::SetSerialBreak() : " << state << std::endl;
#endif
#else
#ifndef DEBUG_RS232
  fprintf(stderr, " Rs232::SetSerialBreak can't get IOCTL\n");
#endif
#endif
  return result;
}

int Rs232::SetSerialTimeouts(long init_read, long while_read)
{
...
int Rs232::SetSerialDTR(int dtr)
{
  int result = E2ERR_OPENFAILED;
  int flags;
...
int Rs232::SetSerialRTS(int rts)
{
  int result = E2ERR_OPENFAILED;
  int flags;
...
int Rs232::SetSerialRTSDTR(int state)
{
  int result = E2ERR_OPENFAILED;
  int flags;
